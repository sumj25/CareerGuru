...
export default class Api extends React.Component {
    ...
    return (
      <Animated.View style={{ marginRight: 18 }}>
        <Card key={item.id}>
          ...
          <Text style={{ lineHeight: 15 }}>Location: {item.location}</Text>
          <Text style={{ lineHeight: 15 }}>Type: {item.type}</Text>
          <Text style={{ lineHeight: 15 }}>Type: {item.title}</Text>
  ...
  renderNoMoreCards() {
    return (
      <View>
        <Text>sorry,no more job around here</Text>
      </View>
    );
  }
  renderFooter = () => {
