...
const ProfileScreen = ({ navigation }) => {
  ...
  const [image, setImage] = useState(
    "https://images.unsplash.com/photo-1611083075509-fba08db259ac?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=334&q=80"
  );
  ...
  const fun = () => {
    ...
    setItems({});
    db.transaction((tx) => {
      tx.executeSql("SELECT * FROM profile_user", [], (tx, results) => {
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={{ alignItems: "center" }}>
        ...
        <Text style={{ fontSize: 18 }}>{items.position}</Text>
      </View>
      <Card
        style={styles.myCard}
        onPress={() => {
          Linking.openURL("mailto:asdf@");
        }}
      >
